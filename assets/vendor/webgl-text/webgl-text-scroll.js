...
(function(){
  ...
  function createProgram(gl, vs, fs){
    var v = createShader(gl, gl.VERTEX_SHADER, vs); if(!v) return null;
    var f = createShader(gl, gl.FRAGMENT_SHADER, fs); if(!f) return null;
  ...
  function initRoot(root){
    ...
    canvas.style.position = 'absolute'; canvas.style.left='0'; canvas.style.top='0'; canvas.style.width='100%'; canvas.style.height='100%'; canvas.style.pointerEvents='none';
    ...
    var buf = gl.createBuffer(); gl.bindBuffer(gl.ARRAY_BUFFER, buf); gl.bufferData(gl.ARRAY_BUFFER, vtx, gl.STATIC_DRAW);
    ...
    function renderTextToCanvas(){
      ...
      lines.forEach(function(line){
        tctx.font = (line.fontWeight) + ' ' + (line.fontSize) + 'px "Humane", system-ui, -apple-system, "Segoe UI", Roboto, Arial, sans-serif';
    ...
    function draw(now){
      ...
      var dt = (now - then) * 0.001; then = now;
  ...
  function init(){
    ...
    roots.forEach(function(r){ try{ initRoot(r); }catch(e){ console.warn(e); } });
  }
